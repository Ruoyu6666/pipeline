...
class MABeMouseDataset(BasePoseTrajDataset):
    ...
    def featurise_keypoints(self, keypoints):
        keypoints = self.normalize(keypoints)  # keypoints (900, 72)
        if self.centeralign:
    ...
    def preprocess(self):
        ...
        for seq_ix, (seq_name, sequence) in enumerate(sequences.items()):
            ...
            if sub_seq_length < 120:
                ...
                pad_length = 120
            pad_vec = np.pad(vec_seq,((pad_length // 2, pad_length - 1 - pad_length // 2), (0, 0)),mode="edge", ) #1919
            seq_keypoints.append(pad_vec)
            keypoints_ids.extend(
                [(seq_ix, i) for i in np.arange(0, len(pad_vec) - sub_seq_length + 1, sliding_window)]  #(0, 1020, 17) ->
            )
