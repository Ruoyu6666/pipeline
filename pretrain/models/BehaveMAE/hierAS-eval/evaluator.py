...
class Paths:
    ...
    CLIP_LENGTHS_FILE = os.getenv("CLIP_LENGTHS_FILE", "eval/metadata/clip_lengths_mouse_triplets.json")
    FRAME_NUMBER_MAP = os.getenv("FRAME_MAP_FILE", "eval/metadata/mouse_frame_number_map.npy")
...
class AIcrowdEvaluator:
    ...
    def _evaluate(self, client_payload, _context={}):
        ...
        if self.task_name == "mabe_mice":
            ...
            Paths.CLIP_LENGTHS_FILE = os.getenv("CLIP_LENGTHS_FILE", "eval/metadata/clip_lengths_mouse_triplets.json")
            Paths.FRAME_NUMBER_MAP = os.getenv("FRAME_MAP_FILE", "eval/metadata/mouse_frame_number_map.npy")
            embedding_max_size = 128
            test_size = 0.1

        elif "Shot" in self.task_name:
            Paths.TASK_INFO_FILE = (f"eval/metadata/{self.task_name}/task_info_{self.task_name}.json")
            ...
            Paths.TASK_INFO_FILE = (f"eval/metadata/{self.task_name}/task_info_hBABEL_val_top_120_60.json")
            Paths.SPLIT_INFO_FILE = (f"eval/metadata/{self.task_name}/split_info_hBABEL_val_filtered.json")

            Paths.CLIP_LENGTHS_FILE = (f"eval/metadata/{self.task_name}/clip_length_hBABEL_val_filtered.json")
            Paths.FRAME_NUMBER_MAP = (f"eval/metadata/{self.task_name}/frame_number_map_val_filtered.npy")
            embedding_max_size = 64
            # embedding_max_size = 128
            test_size = 0.1
