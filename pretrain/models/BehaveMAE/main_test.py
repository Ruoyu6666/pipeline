...
def load_model(args):
    ...
    print("Loading checkpoint from: %s" % chkpt)
...
def extract_hierarchical_embeddings(args):
    ...
    for name, sequence in tqdm(loop):
        ...
        for lv in embeddings:
            ...
            if args.fast_inference: # cut out middle embedding(s)
                embeddings[lv] = embeddings[lv][:, pad // tps, :, :].unsqueeze(1)
                if sliding_window > 1:
                    assert sliding_window <= args.patch_kernel[0]
                    embeddings[lv] = embeddings[lv].repeat_interleave(repeats=sliding_window, dim=1)
            else:
                if args.embedsum:
                ...
                result_embeds = torch.zeros(
                    ...
                    dtype=torch.float32,).to(embeddings[lv].device)
