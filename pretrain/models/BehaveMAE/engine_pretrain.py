...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, data_loader_val: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, log_writer=None, args=None, fp32=False,):
    ...
    for data_iter_step, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=not fp32):
            loss, _, _, _, _ = model(samples, targets, mask_ratio=mask_ratio,
    ...
    if data_loader_val:
        ...
        if epoch % 5 == 0:
            for data_iter_step, samples in enumerate(metric_logger.log_every(data_loader_val, print_freq, header)):
                samples, targets = samples
                samples = samples.to(device, non_blocking=True)
                if targets:
                    targets = [tgt.to(device, non_blocking=True) for tgt in targets]
