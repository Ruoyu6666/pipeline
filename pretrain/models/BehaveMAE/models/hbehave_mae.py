...
class HBehaveMAE(GeneralizedHiera):
    ...
    def __init__(
        ...
        **kwdargs,
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, self.pred_stride * in_chans,)  # predictor
        # --------------------------------------------------------------------------
    ...
    def patch_pixel_label_3d(self, input_vid, t, h, w):
        ...
        # Reshape the tensor to introduce the patch dimensions. Here, we split each dimension T, H, W into 
        # (T//t, t), (H//h, h), (W//w, w) respectively. This gives us a new shape of (B, C, T//t, t, H//h, h, W//w, w).
    ...
    def forward_encoder(self, x: torch.Tensor, mask_ratio: float, mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if mask is None:
            mask = self.get_random_mask(x, mask_ratio)  # [B, #MUs_all] x:[768, 1, 900, 3, 24]
