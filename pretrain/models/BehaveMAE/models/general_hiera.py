...
class MaskUnitAttention(nn.Module):
    ...
    Note: this assumes the tokens have already been flattened and unrolled into mask units. See `Unroll` for more details.
    """

    def __init__(self, dim: int, dim_out: int, heads: int, q_stride: int = 1, 
                 window_size: int = 0, use_mask_unit_attn: bool = False,):
        """
        Args:
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, dim_in: int, dim_out: int, kernel: Tuple[int, ...],
...
class GeneralizedHiera(nn.Module):
    def __init__(
        ...
        **kwargs,  # attention!
        ...
        self.tokens_spatial_shape = [i // s for i, s in zip(input_size, patch_stride)] # [300, 3 ,1]
        num_tokens = math.prod(self.tokens_spatial_shape) # 900

        if non_hierarchical:
            q_strides = [(1, 1, 1)] * len(stages)
            out_embed_dims = [out_embed_dims[0]] * len(stages)

        mask_unit_size = tuple(np.prod(q_strides, axis=0)) # [5, 3, 1]
        flat_mu_size = math.prod(mask_unit_size) # 15
        flat_q_strides = [math.prod(q_stride) for q_stride in q_strides] # [5,3]
        ...
        self.mu_size, self.mask_unit_size = flat_mu_size, mask_unit_size           # (15, [5, 3, 1])
        self.mask_spatial_shape = [i // s for i, s in zip(self.tokens_spatial_shape, self.mask_unit_size)] # [60, 1, 1]
        self.stage_ends = [sum(stages[:i]) - 1 for i in range(1, len(stages) + 1)] # [2, 6, 11]
        ...
        if non_hierarchical:  # non-hierarchical baseline
            for i in range(depth):
                block = HieraBlock(dim=embed_dim, dim_out=embed_dim, heads=num_heads,
                                  mlp_ratio=mlp_ratio, drop_path=dpr[i], norm_layer=norm_layer,
                                  q_stride=1, window_size=flat_mu_size, use_mask_unit_attn=False,)
            ...
            for i in range(depth):
                ...
                block = HieraBlock(dim=embed_dim, dim_out=dim_out, heads=num_heads,
                                   mlp_ratio=mlp_ratio, drop_path=dpr[i], norm_layer=norm_layer,
                                   q_stride=(flat_q_strides[cur_stage - 1] if i in q_pool_blocks else 1),
                                   window_size=flat_mu_size, use_mask_unit_attn=use_mask_unit_attn,)
        ...
        self.projections = nn.ModuleList(
            [
                nn.Linear(self.blocks[i].dim_out, out_embed_dims[ix]) for ix, i in enumerate(self.stage_ends)
            ]
        )
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None,
        ...
        x = self.patch_embed(x, mask=mask.view(x.shape[0], 1, *self.mask_spatial_shape) 
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if return_intermediates and i in self.stage_ends:
                intermediates.append(self.projections[len(intermediates)](self.reroll(x, i, mask=mask)))
