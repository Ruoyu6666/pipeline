...
def undo_windowing(x: torch.Tensor, shape: List[int], mu_shape: List[int]) -> torch.Tensor:
    ...
    Args:
        ...
        shape: current spatial shape, if it were not organized into mask unit windows, e.g. in 2d [B, #MUy*MUy, #MUx*MUx, C].
    ...
    permute = ([0]
               + sum([list(p) for p in zip(range(1, 1 + D), range(1 + D, 1 + 2 * D))], [],) # flatten the list of list to [1, D+1, 2, D+2, 3, D+3, ..., D, 2D]
               + [len(x.shape) - 1]) #[0, 1, D+1, 2, D+2, ... D, 2D, last_dimension]
